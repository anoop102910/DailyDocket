...
function Navbar() {
  ...
  return (
    <>
      <header onClick={()=>toggle(!hide)} className="sticky z-[1000] top-0 left-0 w-full gap-4 h-16 rounded-md shadow-md bg-white flex items-center px-6 justify-between">
        {hide && <Dashboard className={` lg:hidden absolute top-0 left-0 min-h-screen `}  />}
        <Icon onClick={()=>toggle(!hide)} icon="mingcute:menu-fill" className="text-3xl lg:hidden text-slate-700"/>
